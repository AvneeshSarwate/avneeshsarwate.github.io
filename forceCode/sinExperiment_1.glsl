...
void main () {
    ...
    if(mod(segGrid.x, 2.) == mod(segGrid.y, 2.)) xy = rotate(vec2(sin(time/4.), cos(time/4.)), stN.xy, time + noiseVal);
    else xy = rotate(vec2(sin(time/4.), cos(time/4.)), stN.xy, - time - noiseVal);
